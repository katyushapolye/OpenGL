    ...
    */
    ...
    void Renderer::loadShaders(){
        Log::write("[Renderer::loadShaders] - Loading " + std::to_string(SHADER_COUNT) + " shaders...");
        for(int i = 0;i<SHADER_COUNT;i++){
            ...
            if(type == ShaderType::Lit){
                if(!(this->loadedShaders[(ShaderType)i]->loadFromFile("Resources/Shaders/lit_vertex.glsl", "Resources/Shaders/lit_frag.glsl"))){
                    Log::write("[Renderer::loadShaders] - ERROR: Failed to load Lit shader");
                } else {
                    Log::write("[Renderer::loadShaders] - Lit shader loaded OK");
                }
            }
                ...
                if(!(this->loadedShaders[(ShaderType)i]->loadFromFile("Resources/Shaders/instanced_vertex.glsl", "Resources/Shaders/instanced_frag.glsl"))){
                    Log::write("[Renderer::loadShaders] - ERROR: Failed to load Instanced shader");
                } else {
                    Log::write("[Renderer::loadShaders] - Instanced shader loaded OK");
                }
            }
                ...
                if(!(this->loadedShaders[(ShaderType)i]->loadFromFile("Resources/Shaders/volumetric_vertex.glsl", "Resources/Shaders/volumetric_frag.glsl"))){
                    Log::write("[Renderer::loadShaders] - ERROR: Failed to load Raymarch/Volumetric shader");
                } else {
                    Log::write("[Renderer::loadShaders] - Raymarch shader loaded OK");
                }
            }
        }

        // UBO setup logging
        Log::write("[Renderer::loadShaders] - Allocating Matrix UBO (128 bytes, binding 0)");
        glGenBuffers(1, &this->gl_Matrixes_UBO);
        glBindBuffer(GL_UNIFORM_BUFFER, this->gl_Matrixes_UBO);
        glBufferData(GL_UNIFORM_BUFFER, 128, NULL, GL_STATIC_DRAW);
        glBindBufferRange(GL_UNIFORM_BUFFER, 0, this->gl_Matrixes_UBO, 0, 128);
        glBindBuffer(GL_UNIFORM_BUFFER, 0);

        Log::write("[Renderer::loadShaders] - Allocating Camera UBO (binding 1)");
        ...
        Log::write("[Renderer::loadShaders] - UBOs initialized");
    }

    void Renderer::loadSkyBox(){
        Log::write("[Renderer::loadSkyBox] - Loading skybox cubemap...");
        ...
        // After the texture loop, add:
        Log::write("[Renderer::loadSkyBox] - Cubemap faces loaded (" + 
        ...
        1.0f, -1.0f,  1.0f
            ...
            glBufferData(GL_ARRAY_BUFFER,sizeof(skyboxVertices),skyboxVertices,GL_STATIC_DRAW);
                ...
                // After VAO/VBO setup:
        Log::write("[Renderer::loadSkyBox] - Skybox VAO/VBO created");
            ...
            if(!(this->skyboxShader->loadFromFile("Resources/Shaders/skybox_vertex.glsl","Resources/Shaders/skybox_frag.glsl"))){
                ...
                Log::write("[Renderer::loadSkyBox] - Skybox shader loaded OK");
            }
    ...
    void Renderer::loadScreenBuffer(){
        Log::write("[Renderer::loadScreenBuffer] - Creating screen FBO (" + std::to_string(width) + "x" + std::to_string(height) + ")");
        ...
        Log::write("[Renderer::loadScreenBuffer] - ERROR: Screen FBO is incomplete! Status: " + 
                    std::to_string(glCheckFramebufferStatus(GL_FRAMEBUFFER)));
        } else {
            Log::write("[Renderer::loadScreenBuffer] - Screen FBO complete OK");
        }
        ...
        glFramebufferTexture2D(GL_FRAMEBUFFER, GL_DEPTH_STENCIL_ATTACHMENT, GL_TEXTURE_2D, this->gl_Screen_DepthStencil_TEX, 0);  
            if(glCheckFramebufferStatus(GL_FRAMEBUFFER) != GL_FRAMEBUFFER_COMPLETE){
        Log::write("[Renderer::loadScreenBuffer] - ERROR: Depth FBO is incomplete! Status: " + 
                    std::to_string(glCheckFramebufferStatus(GL_FRAMEBUFFER)));
        } else {
            Log::write("[Renderer::loadScreenBuffer] - Depth FBO complete OK");
        }
        ...
        glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, this->gl_Screen_Volumetric_TEX, 0);
            if(glCheckFramebufferStatus(GL_FRAMEBUFFER) != GL_FRAMEBUFFER_COMPLETE){
            Log::write("[Renderer::loadScreenBuffer] - ERROR: Volumetric FBO is incomplete! Status: " + 
                    std::to_string(glCheckFramebufferStatus(GL_FRAMEBUFFER)));
        } else {
            Log::write("[Renderer::loadScreenBuffer] - Volumetric FBO complete OK");
        }
        ...
        glFramebufferTexture2D(GL_FRAMEBUFFER, GL_DEPTH_STENCIL_ATTACHMENT, GL_TEXTURE_2D, this->gl_Screen_Volumetric_DepthStencil_TEX, 0);  
                if(glCheckFramebufferStatus(GL_FRAMEBUFFER) != GL_FRAMEBUFFER_COMPLETE){
            Log::write("[Renderer::loadScreenBuffer] - ERROR: Volumetric Depth FBO is incomplete! Status: " + 
                    std::to_string(glCheckFramebufferStatus(GL_FRAMEBUFFER)));
        } else {
            Log::write("[Renderer::loadScreenBuffer] - Volumetric Depth FBO complete OK");
        }
    ...
    void Renderer::loadShadowMap() {
        ...
        //generate 32 textures for the maps
                ...
                Log::write("[Renderer::loadShadowMap] - Initializing shadow maps: " + 
                std::to_string(MAX_SHADOWCASTERS) + " casters, " +
                std::to_string(gl_ShadowMap_Resolution) + "x" + 
                std::to_string(gl_ShadowMap_Resolution) + " resolution");
        ...
        for(int i = 0;i<this->MAX_SHADOWCASTERS;i++){
            ...
            if(glCheckFramebufferStatus(GL_FRAMEBUFFER) != GL_FRAMEBUFFER_COMPLETE){
                Log::write("[Renderer::loadShadowMap] - ERROR: Shadow FBO " + std::to_string(i) + " is incomplete!");
            }
        }
        Log::write("[Renderer::loadShadowMap] - Shadow FBOs created OK");
        ...
        if(!(this->shadowMapShader->loadFromFile("Resources/Shaders/shadow_vertex.glsl",
            ...
            Log::write("[Renderer::loadShadowMap] - Shadow map shader loaded OK");
        }
    ...
    //we only do this once per scene unless we add more objects to the scene which we will check later using some flags
        if(this->hasSortedGroups == false){
            ...
            int totalOpaque = 0;
            for(auto& group : opaqueShaderDrawGroups) totalOpaque += group.second.size();
            Log::write("[Renderer::sortSceneModels] - Sorted: " + std::to_string(totalOpaque) +
                    " opaque, " + std::to_string(transparentDrawGroups.size()) + " transparent");
        }
    ...
    void Renderer::processInput(){
        ...
        DirectionalLight* light = (DirectionalLight*)this->loadedScene->getLights().at(LightType::DIRECTIONAL)[0].get();
    ...
    void Renderer::setupShaderLighting(Shader* shader){
        ...
        shader->setUniform("directionalLightCount", (int)this->loadedScene->getLights().at(LightType::DIRECTIONAL).size());
        shader->setUniform("pointLightCount", (int)this->loadedScene->getLights().at(LightType::POINT).size());
        shader->setUniform("spotLightCount", (int)this->loadedScene->getLights().at(LightType::SPOT).size());

        for(int i = 0; i < this->loadedScene->getLights().at(LightType::POINT).size(); i++){
            PointLight* light = static_cast<PointLight*>(this->loadedScene->getLights().at(LightType::POINT)[i].get());
        ...
        for(int i = 0; i < this->loadedScene->getLights().at(LightType::DIRECTIONAL).size(); i++){
            DirectionalLight* light = static_cast<DirectionalLight*>(this->loadedScene->getLights().at(LightType::DIRECTIONAL)[i].get());
        ...
        for(int i = 0; i < this->loadedScene->getLights().at(LightType::SPOT).size(); i++){
            SpotLight* light = static_cast<SpotLight*>(this->loadedScene->getLights().at(LightType::SPOT)[i].get());
    ...
    void Renderer::shadowPass(){
        ...
        for(int i = 0;i<this->activeShadowCasters;i++){
            ...
            if(i >= this->shadowCasters.size()){
                Log::write("[Renderer::shadowPass] - ERROR: Shadow caster index " + 
                        std::to_string(i) + " out of range! shadowCasters.size()=" +
                        std::to_string(this->shadowCasters.size()));
                break;
            }
        ...
        for(int i = 0;i<loadedShaders.size();i++){
            for(auto it = opaqueShaderDrawGroups[(ShaderType) i].begin() ;it != opaqueShaderDrawGroups[(ShaderType) i].end();it++){
                ...
                if(drawable->getType() == DrawableType::MODEL){
                    ...
                    glStencilMask(0x00);
                    glDisable(GL_CULL_FACE); //we have billboards here
                    ...
                    model->draw(shader);
                    glEnable(GL_CULL_FACE); //we have billboards here
    ...
    void Renderer::dispose(){
        Log::write("[Renderer::dispose] - Disposing renderer...");

        Log::write("[Renderer::dispose] - Shutting down ImGui");
        ImGui_ImplOpenGL3_Shutdown();
        ImGui_ImplGlfw_Shutdown();
        ImGui::DestroyContext();

        Log::write("[Renderer::dispose] - Releasing OpenGL resources");


        Log::write("[Renderer::dispose] - Destroying GLFW window");
        glfwDestroyWindow(this->gl_Window);
        glfwTerminate();

        Log::write("[Renderer::dispose] - Renderer destroyed gracefully");
    }


    void Renderer::loadScene(Scene* scene){
    Log::write("[Renderer::loadScene] - Loading scene...");
        this->loadedScene = scene;
        this->hasSortedGroups = false;

        int dirCount  = scene->getLights().at(LightType::DIRECTIONAL).size();
        int spotCount = scene->getLights().at(LightType::SPOT).size();
        int ptCount   = scene->getLights().at(LightType::POINT).size();
        int modelCount = scene->getModels().size();

        Log::write("[Renderer::loadScene] - Models: "      + std::to_string(modelCount));
        Log::write("[Renderer::loadScene] - Dir lights: "  + std::to_string(dirCount));
        Log::write("[Renderer::loadScene] - Spot lights: " + std::to_string(spotCount));
        Log::write("[Renderer::loadScene] - Point lights: "+ std::to_string(ptCount));

        this->activeShadowCasters = dirCount + spotCount;
        Log::write("[Renderer::loadScene] - Active shadow casters: " + std::to_string(this->activeShadowCasters));

        if(this->activeShadowCasters > this->MAX_SHADOWCASTERS){
            Log::write("[Renderer::loadScene] - WARNING: activeShadowCasters (" + 
                    std::to_string(this->activeShadowCasters) + ") exceeds MAX_SHADOWCASTERS (" +
                    std::to_string(this->MAX_SHADOWCASTERS) + ")! Will be clamped.");
        }

           const auto& dirLights = scene->getLights().at(LightType::DIRECTIONAL);
